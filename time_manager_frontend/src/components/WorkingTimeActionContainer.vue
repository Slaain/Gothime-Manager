...
<template>
  ...
  >
    <div class="border-b">
      <!-- Bouton "Créer" -->
      <button
        @click="setFormMode('create')"
        :class="[
          'focus:outline-none m-2 border border-purple-800    hover:bg-purple-800 hover:text-white focus:ring-purple-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-2 ',
...
export default {
  ...
  methods: {
    ...
    updateWorkingTime() {
      ...
      axios
        .put(
          "http://localhost:4000/api/workingtime/2",
          {
            working_time: {
              start: this.startTime,
              end: this.endTime,
    ...
    getWorkingTime() {
      axios
        .get("http://localhost:4000/api/workingtime/1/2")
        .then((response) => {
