...
<template>
  <div
    class="w-[40%] shadow-md rounded-lg flex flex-col justify-center items-center"
  >
    <div class="border-b">
      <!-- Bouton pour afficher le temps de travail d'un utilisateur -->
      <button
        @click="setFormMode('show')"
        :class="[
          'focus:outl<ine-none m-2 border border-purple-800 bg-white focus:text-white focus:bg-purple-800 hover:bg-purple-800 hover:text-white focus:ring-purple-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-2',
      ...
      <div v-if="workingTime">
        <table class="min-w-full bg-white">
          <thead class="border-b">
            <tr>
              <th class="px-4 py-2 text-left text-gray-600">Session</th>
          ...
          <tbody>
            ...
            >
              ...
              <td class="px-4 py-2">
                {{ calculateDuration(wt.start, wt.end) }}
              </td>
            </tr>
          </tbody>
        </table>
...
export default {
  ...
  methods: {
    ...
    showWorkingTime() {
      ...
      axios
        .get(`http://localhost:4000/api/workingtimes/1/1`, {
          headers: {
