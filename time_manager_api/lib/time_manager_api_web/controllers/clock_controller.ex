defmodule TimeManagerApiWeb.ClockController do
  ...
  alias TimeManagerApi.QRCodeService
    ...
    def beep(conn, %{"user_id" => user_id, "organization_id" => organization_id, "token" => token}) do
      # Convertir organization_id en entier
      organization_id = String.to_integer(organization_id)

      # Vérifier si le token correspond au QR code de l'organisation
      case QRCodeService.validate_token(organization_id, token) do
        {:ok, :valid} ->
          # Continuer la logique du beep après validation du token
