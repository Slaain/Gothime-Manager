...
export default function GothamNeedsYouScreen() {
    ...
    const fetchDataForViewMode = async (mode) => {
        ...
        switch (mode) {
            case 'daily':
                url = `http://10.79.216.202:4000/api/workingtimes/daily/${userId}`;
                break;
            case 'weekly':
                url = `http://10.79.216.202:4000/api/workingtimes/weekly/${userId}`;
                break;
            case 'monthly':
                url = `http://10.79.216.202:4000/api/workingtimes/monthly/${userId}`;
                break;
            default:
                return;
        }
    ...
    const getWorkingTimes = async () => {
        try {
            ...
            const response = await axios.get(`http://10.79.216.202:4000/api/workingtimes/${userId}`);
